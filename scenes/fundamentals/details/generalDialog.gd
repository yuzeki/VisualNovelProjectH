...
func bg_change(path: String):
	var colorbg = get_node('background').get_node("colorBackground")
	if path.begins_with("<"):
		var parseTxt = path.substr(1, path.length() - 2)
		var paramList = parseTxt.split_floats(",")
		if paramList.size() < 3:
			vn.error("p")
			return
		var parseColor = Color(paramList[0],paramList[1],paramList[2])
